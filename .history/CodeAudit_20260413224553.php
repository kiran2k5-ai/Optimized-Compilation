...
class CodeAudit {
    ...
    private static function store_in_database($attempt_id, $userid, $code, $hash, $timestamp) {
        ...
        $stmt = $mysqli->prepare(
            "INSERT INTO mdl_code_audit 
            (attempt_id, userid, code_hash, code_size, ip_address, user_agent, timestamp, action) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        );
        ...
        $stmt->bind_param(
            'iisisss',
            $attempt_id, $userid, $code_size, $hash, $ip_address, $user_agent, $timestamp, $action
        );
