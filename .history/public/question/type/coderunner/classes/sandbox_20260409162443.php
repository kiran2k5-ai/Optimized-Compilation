    ...
    public static function get_instance($sandboxextname) {
        ...
        if (array_key_exists($sandboxextname, $boxes)) {
            ...
            if (!class_exists($classname)) {
                throw new Exception("Sandbox class '$classname' not found after requiring '$filename'");
            }
            $sb = new $classname();
            return $sb;
        } else {
            throw new Exception("Sandbox '$sandboxextname' is not enabled. Available: " . implode(', ', array_keys($boxes)));
        }
    }
