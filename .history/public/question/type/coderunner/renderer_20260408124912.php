...
class qtype_coderunner_renderer extends qtype_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa, question_display_options $options) {
        ...
        // Load Pyodide for local code execution
        // Enable configuration
        require_once(__DIR__ . '/enable_pyodide.php');
        
        // Load Pyodide JavaScript library from CDN
        $pyodide_cdn = 'https://cdn.jsdelivr.net/pyodide/v0.23.0/full/pyodide.js';
        $this->page->requires->js($pyodide_cdn, true);  // true = async loading
        
        // Load our custom executor
        $this->page->requires->js('/question/type/coderunner/pyodide_executor.js');
        
        // Load Pyodide styles (commented out - file may not exist)
        // $this->page->requires->css('/question/type/coderunner/pyodide_styles.css');
        $qid = $question->id;
        ...
        // Initialize Pyodide check button (executes in browser, blocks server submission)
        $this->page->requires->js_call_amd(
            'qtype_coderunner/pyodide_checkbutton',
            'initPyodideCheckButton',
            [$qid]
        );

        // Block any form submission attempts
        $this->page->requires->js_call_amd(
            'qtype_coderunner/pyodide_checkbutton',
            'blockServerSubmission',
            []
        );
