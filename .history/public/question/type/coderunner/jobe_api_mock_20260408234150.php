...
class jobe_api_mock {
    ...
    public static function run_code($code, $input, $language, $timeout = 10) {
        ...
        // Return a response with standard CodeRunner/Jobe format
        // Tests expect top-level stdout/stderr/returncode fields
        return array(
            'status' => 0,  // Success (0 = success, non-zero = error)
            'stdout' => '',  // Output from the code
            'stderr' => '',  // Error output
            'returncode' => 0,  // Program exit code
            'cputime' => 0,
            'walltime' => 0,
            'signal' => null,
            'max_memory' => 0,
