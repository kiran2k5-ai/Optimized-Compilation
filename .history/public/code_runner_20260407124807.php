...
// Check if this is an API request (GET without setup parameter = API request)
$isApiRequest = isset($_GET['api']) || $_SERVER['REQUEST_METHOD'] !== 'GET' || isset($_GET['question_id']) || isset($_GET['action']) || (!isset($_GET['setup']) && $_SERVER['REQUEST_METHOD'] === 'GET' && strpos($_SERVER['REQUEST_URI'], '?') !== false);

// If GET request with no parameters - it's also API request for loading questions
if ($_SERVER['REQUEST_METHOD'] === 'GET' && !isset($_GET['api']) && !isset($_GET['question_id']) && !isset($_GET['action']) && !isset($_GET['setup'])) {
    $isApiRequest = true;
}
