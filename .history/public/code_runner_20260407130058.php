...
// Handle quiz setup with questions from URL parameter (like Moodle sending quiz data)
if (isset($_GET['setup']) && !empty($_GET['setup'])) {
    $questionsData = json_decode(base64_decode($_GET['setup']), true);
    if ($questionsData && is_array($questionsData)) {
        // Save questions to file
        file_put_contents($questionsFile, json_encode($questionsData, JSON_PRETTY_PRINT));
        // Don't redirect - let it load the page with questions immediately
    }
}

// Check if this is an API request (any query param except 'setup' = API, or any POST/DELETE)
$isApiRequest = $_SERVER['REQUEST_METHOD'] !== 'GET' || // POST, DELETE, etc
                (isset($_GET['api']) || isset($_GET['question_id']) || isset($_GET['action'])); // Explicit API params
...
<body>
    ...
    <script>
        ...
        async function loadAllQuestions() {
            try {
                const response = await fetch(API_URL + '?api=1');
        ...
        function displayQuestion() {
            ...
            const savedCode = localStorage.getItem('question_' + currentQuestion.id) || '';
            ...
            document.getElementById('code-editor').addEventListener('input', function() {
                localStorage.setItem('question_' + currentQuestion.id, this.value);
            });
        ...
        // Display test cases (only non-hidden ones)
        function displayTestCases() {
            ...
            currentQuestion.tests.forEach((test, index) => {
                ...
                div.innerHTML = `
                    <strong>Test Case ${visibleCount}:</strong><br>
...
`;
        ...
        // Display final results (minimal - just confirmation)
