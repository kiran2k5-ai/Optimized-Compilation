...
if (preg_match('|/restapi/([a-z]+)|i', $path_info, $matches)) {
    ...
    if (strtolower($resource) === 'runs' && ($_SERVER['REQUEST_METHOD'] === 'POST' || $_SERVER['REQUEST_METHOD'] === 'PUT')) {
        ...
        if (($language === 'python3' || $language === 'python') && !empty($sourcecode)) {
            try {
                // Create a unique temporary file path using microtime
                $tmpdir = sys_get_temp_dir();
                if (!is_dir($tmpdir)) {
                    $tmpdir = '.';
                }
                
                $tmpfile = $tmpdir . DIRECTORY_SEPARATOR . 'coderunner_' . uniqid() . '.py';
                
                // Write the code to a temporary file
                if (file_put_contents($tmpfile, $sourcecode) === false) {
                    throw new Exception("Could not write to temporary file");
                }
                
                if (!empty($input)) {
                    // Write input to a file for piping
                    $tmpinputfile = $tmpdir . DIRECTORY_SEPARATOR . 'coderunner_input_' . uniqid();
